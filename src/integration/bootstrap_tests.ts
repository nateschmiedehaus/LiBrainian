 ...
 *
 * Provides a test runner that validates LiBrainian can correctly index
 * and query its own codebase. Used for self-validation.
 *
 * @example
 * ```typescript
 * import { createLiBrainian } from 'librainian';
 * import { createBootstrapTestRunner } from 'librainian/integration';
 *
 * const librainian = await createLiBrainian({ workspace: '/path/to/project' });
 * const runner = createBootstrapTestRunner(librainian);
 ...
 * - self-indexing: Verify LiBrainian indexes its own source files
 * - query: Query for known LiBrainian components
...
export function createBootstrapTestRunner(librainianInstance: any): BootstrapTestRunner {
  ...
  // Get workspace from librainian config
  const getWorkspace = (): string => {
    return librainianInstance.config?.workspace || process.cwd();
  };
  ...
  addTest('query', 'queries by file path', async () => {
    const storage = librainianInstance.getStorage();
    const functions = await storage.getFunctionsByPath(path.resolve(getWorkspace(), 'src/api/librainian.ts'));
