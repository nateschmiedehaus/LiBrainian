...
/**
 * Resolves the best available LLM transport:
 *  1. API transport via ANTHROPIC_API_KEY (most reliable, works everywhere)
 *  2. CLI transport via `claude` subprocess (legacy, only outside nested sessions)
 */
export function createApiLlmServiceFactory(): LlmServiceFactory {
  return async () => new ApiLlmService();
}

/**
 * Smart factory that picks the best transport based on environment:
 *  - If ANTHROPIC_API_KEY is set -> API transport
 *  - Otherwise -> CLI transport (from cli_llm_service.ts)
 */
export function createAutoLlmServiceFactory(): LlmServiceFactory {
  return async () => {
    ...
    logInfo('LLM transport: using Claude CLI subprocess');
