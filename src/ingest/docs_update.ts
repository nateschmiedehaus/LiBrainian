...
function generateLibrarianSection(
  ...
  const lines: string[] = [
    ...
    '```',
    '',
    '### When to Use LiBrainian Tools',
    '',
    '**ALWAYS call `query` before:**',
    '- Starting tasks in unfamiliar repository areas',
    '- Refactoring behavior that spans multiple files',
    '- Debugging failures with unclear upstream/downstream causes',
    '- Estimating blast radius before changing signatures',
    '- Writing tests for modules you have not inspected recently',
    '',
    '**Use `intentType` values to route correctly:**',
    '- `understand`: architecture, ownership, and behavior questions',
    '- `debug`: trace failures, call chains, and likely root causes',
    '- `impact`: estimate what changes if a symbol/module is modified',
    '- `refactor`: safe structural changes and rename planning',
    '',
    '**Set `depth` deliberately:**',
    '- `L0`: exact local matches only (narrow context)',
    '- `L1`: default for most tasks (balanced recall/cost)',
    '- `L2`: include neighbors and transitive relationships',
    '- `L3`: deep cross-module context for complex incidents',
    '',
    '**Do NOT use `query` when:**',
    '- You already know the exact file path and need raw source only',
    '- You need live runtime state, shell output, or network data',
    '- You are only executing commands/tests rather than retrieving code context',
    '',
    '### Available MCP Tools (quick summary)',
    '',
    '- `query(intent, intentType)`: retrieve grounded multi-file context before edits',
    '- `find_symbol(query)`: discover IDs for functions, modules, claims, runs, or compositions',
    '- `get_context_pack_bundle(entityIds, maxTokens)`: fetch paginated context packs for known entities',
    '- `verify_claim(claimId)`: re-check claim grounding against current evidence',
    '- `status()`: check workspace readiness/index state before retrieval',
    '- `bootstrap(workspace)`: build or refresh the index before advanced tools',
    '',
