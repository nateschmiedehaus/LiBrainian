...
describe('MCP synthesize_plan tool', () => {
  it('creates a plan entry and appends audit log event', async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'librainian-synthesize-plan-'));
  ...
  it('retrieves a stored plan by plan_id via status', async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'librainian-synthesize-status-'));
