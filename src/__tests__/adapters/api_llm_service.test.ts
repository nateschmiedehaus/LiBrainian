...
import { ApiLlmService, isInsideClaudeCodeSession, createAutoLlmServiceFactory } from '../../adapters/api_llm_service.js';
...
describe('ApiLlmService', () => {
  ...
  describe('constructor', () => {
    it('throws when no API key is available', () => {
      ...
      expect(() => new ApiLlmService()).toThrow('ANTHROPIC_API_KEY is required');
    });

    it('accepts explicit API key parameter', () => {
      delete process.env.ANTHROPIC_API_KEY;
      const service = new ApiLlmService('sk-ant-explicit-key');
      expect(service).toBeInstanceOf(ApiLlmService);
    });
  ...
  describe('checkClaudeHealth', () => {
    ...
    it('returns unauthenticated when key format is invalid', async () => {
      const service = new ApiLlmService('not-a-real-key');
      const health = await service.checkClaudeHealth();
      expect(health.available).toBe(true);
...
describe('createAutoLlmServiceFactory', () => {
  ...
  it('returns ApiLlmService when ANTHROPIC_API_KEY is set', async () => {
  ...
  it('falls back to CliLlmService when no API key', async () => {
    delete process.env.ANTHROPIC_API_KEY;
    const factory = createAutoLlmServiceFactory();
    const service = await factory();
    // Should not be ApiLlmService
    expect(service).not.toBeInstanceOf(ApiLlmService);
