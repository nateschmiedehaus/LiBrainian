...
async function loadEvalCorpus(corpusPath: string, corpusPaths?: string[]): Promise<EvalCorpus> {
  ...
  for (const root of roots) {
    ...
    let repoEntries: Array<{ isDirectory: () => boolean; isSymbolicLink: () => boolean; name: string }> = [];
    try {
    ...
    for (const entry of repoEntries) {
      ...
      let groundTruth: { version?: string; repoId?: string; queries?: GroundTruthQuery[] };
      try {
        manifest = await readJson<RepoManifest>(join(repoRoot, 'manifest.json'));
        groundTruth = await readJson<{ version?: string; repoId?: string; queries?: GroundTruthQuery[] }>(
