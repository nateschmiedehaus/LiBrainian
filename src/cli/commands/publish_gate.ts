...
async function evaluateOptionalJsonSignal(input: {
  id: string;
  filePath: string;
  maxArtifactAgeHours: number;
  overrideProvided?: boolean;
  evaluate: (value: unknown) => { ok: boolean; message: string; hint?: string } | Promise<{ ok: boolean; message: string; hint?: string }>;
}): Promise<PublishGateSignal | null> {
  try {
    await stat(input.filePath);
  } catch {
    if (!input.overrideProvided) return null;
  }
  return evaluateJsonSignal({
    id: input.id,
...
async function collectReleaseSignals(input: {
  ...
  const signalResults = await Promise.all([
    ...
    evaluateJsonSignal({
      ...
      evaluate: (value) => {
        ...
        if (report.passed !== true) {
          return {
            ok: false,
            message: 'Testing-discipline report did not pass',
            hint: 'Resolve failed blocking checks or warnings before publish.',
          };
        }
        if ((report.summary?.failedBlockingChecks ?? 1) !== 0 || (report.summary?.warningChecks ?? 1) !== 0) {
          return {
            ok: false,
            message: `Testing-discipline report is not clean (blocking=${report.summary?.failedBlockingChecks ?? 'missing'}; warnings=${report.summary?.warningChecks ?? 'missing'})`,
            hint: 'Release evidence requires zero failed blocking checks and zero warnings.',
          };
        }
    ...
    evaluateJsonSignal({
      ...
      evaluate: (value) => {
        ...
        if (report.summary?.publishReady !== true) {
          return {
            ok: false,
            message: 'Testing tracker is not publish-ready',
            hint: 'Resolve open or unknown tracker flaws before publish.',
          };
        }
  ...
  return signalResults.filter((signal): signal is PublishGateSignal => signal !== null);
}
...
export async function publishGateCommand(options: PublishGateCommandOptions): Promise<void> {
  ...
  const readOptionalReleaseInput = async (filePath: string): Promise<string | null> => {
    try {
  ...
  const [gatesRaw, statusMarkdownRaw] = await Promise.all([
    readOptionalReleaseInput(gatesFilePath),
    readOptionalReleaseInput(statusFilePath),
  ]);
  const gates = gatesRaw ? JSON.parse(gatesRaw) as GatesFile : {};
  const statusMarkdown = statusMarkdownRaw ?? '';
