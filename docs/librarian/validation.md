...
## Publish Evidence Non-Negotiables
- `REAL_AGENT_REAL_LIBRAINIAN_ONLY`: publish and launch gates accept only evidence generated by real agents acting on the actual LiBrainian repository.
- `NO_SYNTHETIC_OR_REFERENCE_FOR_RELEASE`: synthetic fixtures and reference workers are diagnostic-only artifacts and cannot satisfy publish gates.
- `NO_RETRY_NO_FALLBACK_FOR_RELEASE_EVIDENCE`: release evidence is invalid if artifacts show retry, fallback, degraded, unavailable, or unverified execution paths.
- `PERFECT_RELEASE_EVIDENCE_ONLY`: release evidence is binary pass/fail; acceptable state is 100% strict pass with zero strict-failure markers.
- If those conditions are not met, publish state is fail-closed.

### Enforcement Notes
- Publish gate validates full artifact schemas (`LiveFireTrialReport.v1`, `ExternalRepoSmokeRunArtifact.v1`) and rejects pointer-only or summary-only payloads.
- A/B release evidence must be `agent_command` only, include control+treatment worker groups, and show treatment context sourced through LiBrainian.
- External smoke release evidence must represent multi-repo coverage; single-repo smoke is diagnostic only.
- Agentic use-case review evidence must come from docs-derived UC intents executed on real external repos (`AgenticUseCaseReviewReport.v1`).
- Agentic use-case review must execute progressive dependency ladders from `USE_CASE_MATRIX.md` before target UC examples; targets without dependency-ready prerequisites are release-invalid.
- Release publish is blocked when warning budget is non-zero (`publish-gate --zero-warning`).

### Agent Session Qualification Invocation
- Full strict qualification chain: `npm run test:agentic:strict`
...
- LiBrainian directory bootstrap executed
...
## LiBrainian Directory Bootstrap (Required)
...
- Use `npm run LiBrainian -- bootstrap --scope LiBrainian --mode fast --llm-provider codex --llm-model gpt-5.1-codex-mini` for a focused run.
...
- Store the matrix as an audit artifact:
  `state/audits/LiBrainian/coverage/uc_method_scenario_matrix.json` (or .csv).
- Fail the audit if any UC lacks a PASS entry for a method family + scenario.
- Use `LiBrainian coverage --strict` to generate and enforce this matrix.
...
## Scenario Validation
Each high-priority scenario in `docs/LiBrainian/scenarios.md` must have:
...
## LiBrainian-Specific Test Categories (Agentic Testing Philosophy)
...
The LiBrainian must correctly represent the codebase. These are deterministic invariants.
...
The LiBrainian must surface the right evidence. These require evaluation datasets.
...
The LiBrainian must handle adversarial and edge-case inputs safely.
...
| **Rate limit** | Provider returns 429 | Mark run failed; no retry/fallback acceptance in release evidence |
...
## Closed-Loop Tests (LiBrainian + Agent)

Test the coupled system, not just the organs.

### Ablation Tests

Run the same episodes with LiBrainian: fully enabled and disabled for controlled comparison.
For release qualification, degraded/fallback episodes are diagnostic-only and cannot satisfy gates.

| Condition | Expected Outcome |
|-----------|------------------|
| Full LiBrainian | Best performance baseline |
| Disabled | Significant performance drop |
| Decorative LiBrainian | Performance unchanged → **FAIL** (LiBrainian adds no value) |
