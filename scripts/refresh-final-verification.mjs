...
function buildPhase21(samples, previousPhase21, benchmarkPlan) {
  ...
  return {
    p50LatencyMs: asNumber(previousPhase21?.p50LatencyMs, 0),
    p99LatencyMs: asNumber(previousPhase21?.p99LatencyMs, 1),
...
async function main() {
  ...
  const phase21 = buildPhase21(rawSamples, previous?.validation_results?.phase21, benchmarkPlan);
  ...
  const next = {
    ...
    targets: {
      ...
      phase21: {
        ...
        p50LatencyMs: asNumber(previous?.targets?.phase21?.p50LatencyMs, 500),
        p99LatencyMs: asNumber(previous?.targets?.phase21?.p99LatencyMs, 2000),
